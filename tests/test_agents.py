...
def test_research_topic_tool() -> None:
    """Test research_topic tool function."""
    result: dict[str, Any] = research_topic("Civil Rights Movement")
...
def test_write_content_tool() -> None:
    """Test write_content tool function."""
    research_data: dict[str, str] = {"topic": "Liberation Movements"}
    result: dict[str, Any] = write_content(research_data)
...
def test_researcher_agent_exists() -> None:
    ...
    assert len(researcher_agent.tools) > 0  # pyright: ignore[reportUnknownArgumentType,reportUnknownMemberType]


def test_writer_agent_exists() -> None:
    """Test that writer agent is properly configured."""
    assert writer_agent.name == "writer"
    assert writer_agent.model == "gemini-2.5-flash"
    assert len(writer_agent.tools) > 0  # pyright: ignore[reportUnknownArgumentType,reportUnknownMemberType]
