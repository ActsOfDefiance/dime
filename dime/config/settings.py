...
class GoogleADKSettings(BaseSettings):
    ...
    api_key: SecretStr = Field(
        ...,
        description="Google API key for ADK"
    )
...
class AgentSettings(BaseSettings):
    ...
    max_llm_calls: int = Field(
        default=500,
        description="Maximum LLM calls per session"
    )
    ...
    enable_tracing: bool = Field(
        default=False,
        description="Enable agent tracing"
    )
...
class LogfireSettings(BaseSettings):
    ...
    token: Optional[SecretStr] = Field(
        default=None,
        description="Logfire API token (optional for development)"
    )
    ...
    send_to_logfire: bool = Field(
        default=True,
        description="Enable sending logs to Logfire"
    )

    @model_validator(mode="after")
    def validate_send_to_logfire(self):
        """Disable Logfire if no token provided."""
        if self.token is None or (isinstance(self.token, SecretStr) and not self.token.get_secret_value().strip()):
...
class ApplicationSettings(BaseSettings):
    ...
    name: str = Field(
        default="Dime Content Creation System",
...
class StorageSettings(BaseSettings):
    ...
    base_path: str = Field(
        default="./storage",
        description="Base storage directory path"
    )
    research_path: str = Field(
        default="./storage/research",
        description="Research documents storage path"
    )
    articles_path: str = Field(
        default="./storage/articles",
        description="Generated articles storage path"
    )
    graphics_path: str = Field(
        default="./storage/graphics",
        description="Generated graphics storage path"
    )
    exports_path: str = Field(
        default="./storage/exports",
        description="Final exports storage path"
    )
...
class DimeSettings(BaseSettings):
    ...
    def _validate_environment(self) -> None:
        ...
        if not self.google_adk.api_key or not self.google_adk.api_key.get_secret_value().strip():
            errors.append("GOOGLE_ADK_API_KEY is required")

        # Check auth secrets in production
        if self.app.environment == "production":
            if not self.auth.jwt_secret_key or not self.auth.jwt_secret_key.get_secret_value().strip():
                errors.append("AUTH_JWT_SECRET_KEY is required in production")

            if not self.auth.google_oauth_client_secret or not self.auth.google_oauth_client_secret.get_secret_value().strip():
